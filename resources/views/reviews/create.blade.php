    ...
    <section class="text-gray-600 body-font relative">
      <div class="container px-5 py-5 mx-auto">
        <div class="flex flex-col text-center w-full mb-12">
          <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-indigo-900 text-bold"><span class="underline">{{ $post_c->title }}</span>をレビュー</h1>
          <p class="lg:w-2/3 mx-auto leading-relaxed text-base">投稿で学んだことを共有しよう！</p>
        </div>
        <div class="lg:w-1/2 md:w-2/3 mx-auto">
        <x-auth-validation-errors class="mb-4" :errors="$errors" />
        <form method="post" action="{{ route('reviews.store', [$post_c->id]) }}">
            @csrf
          <div class="flex flex-wrap -m-2">
            ...
            <div class="p-2 w-full flex justify-around mt-4">
              ...
              <input type="hidden" id="post_id" name="post_id" value="{{ $post_c->id }}" >  
              <button type="button" onclick="location.href='{{ route('posts.show', [$post_c->id]) }}'" class="bg-gray-200 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded text-lg">戻る</button>
